...
describe Sherlock::Search do
  ...
  it "indexes a record" do
    ...
    sleep 1.4
    result = Sherlock::Search.perform_query(realm, "hot")
  ...
  it "does not find something thats not there" do
    Sherlock::Search.index record
    sleep 1.4
    result = Sherlock::Search.perform_query(realm, "lukewarm")
  ...
  it "deletes the whole index" do
    ...
    Sherlock::Search.delete_index(realm)
    sleep 1.4
    lambda { Sherlock::Search.perform_query(realm, "hot") }.should raise_error(Pebblebed::HttpError)
  end
