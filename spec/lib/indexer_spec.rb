...
describe Sherlock::Indexer do
  ...
  it "builds an index record from payload" do
    expected_record =  {"document.app"=>"hot", "id"=>"post.card:hell.pitchfork$1", "klass_0_"=>"post", "klass_1_"=>"card", "label_0_"=>"hell", "label_1_"=>"pitchfork", "oid_"=>"1", "realm"=>"hell", "uid"=>"post.card:hell.pitchfork$1"}
  ...
  it "builds one index record for every path entry in payload" do
    first_expected_record =  {"document.app"=>"hot", "id"=>"post.card:hell.tools.pitchfork$1", "klass_0_"=>"post", "klass_1_"=>"card", "label_0_"=>"hell", "label_1_"=>"trademarks", "label_2_"=>"pitchfork", "oid_"=>"1", "realm"=>"hell", "uid"=>"post.card:hell.trademarks.pitchfork$1"}
    last_expected_record =  {"document.app"=>"hot", "id"=>"post.card:hell.tools.pitchfork$1", "klass_0_"=>"post", "klass_1_"=>"card", "label_0_"=>"hell", "label_1_"=>"tools", "label_2_"=>"pitchfork", "oid_"=>"1", "realm"=>"hell", "uid"=>"post.card:hell.tools.pitchfork$1"}
  ...
  context 'index a post with multiple paths' do
    ...
    it 'it deletes the correct index' do
      ...
      query = Sherlock::Query.new(nil, :uid => 'post.card:hell.tools.pitchfork$1')
      result = Sherlock::Search.query(realm, :source => query.to_json)
      result['hits']['total'].should eq 1
      result['hits']['hits'].first['_id'].should eq 'post.card:hell.tools.pitchfork$1'

      query = Sherlock::Query.new(nil, :uid => 'post.card:hell.trademarks.pitchfork$1')
      result = Sherlock::Search.query(realm, :source => query.to_json)
